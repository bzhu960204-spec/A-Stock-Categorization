...
function Get-Jar {
  Get-ChildItem (Join-Path $backendDir 'target') -Filter 'stock-classifier-*.jar' -ErrorAction SilentlyContinue |
...
$stateFile = Join-Path $scriptRoot '.stockcard-prod-state.json'
@{
  port      = $Port
...
if (-not $NoBrowser) {
  Start-Job -Name 'stockcard-prod-open' -ScriptBlock {
    param([int]$Port)
    for ($i = 0; $i -lt 180; $i++) {
...
Write-Host ""
Write-Host "==============================================="
Write-Host " A Stock Card (prod) starting"
...
# Run in the foreground with cwd = backend so H2 (./data) and logs (./logs)
# match the dev flow. Program args (not env) carry the port + dist so they
# always reach Spring reliably.
...
try {
  ...
  Pop-Location
  Get-Job -Name 'stockcard-prod-open' -ErrorAction SilentlyContinue | Remove-Job -Force -ErrorAction SilentlyContinue
